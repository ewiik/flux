...
Oxygenplot <- ggplot(Oxygen.pdatnorm, aes(x = Oxygen_ppm, y = Fitted)) +
  papertheme +
  annotate("rect", xmin=Oxygenquants[1], xmax=Oxygenquants[2], ymin=-Inf, ymax=Inf, alpha = 0.1, fill='gray60') +
  geom_line() +
  geom_ribbon(aes(ymin = Fittedminus, ymax = Fittedplus), 
              alpha = 0.25) +  
...
DOCplot <- ggplot(DOC.pdatnorm, aes(x = DOC_mg_L, y = Fitted)) +
  papertheme +
  annotate("rect", xmin=DOCquants[1], xmax=DOCquants[2], ymin=-Inf, ymax=Inf, alpha = 0.1, fill='gray60') +
  geom_line() +
  geom_ribbon(aes(ymin = Fittedminus, ymax = Fittedplus), 
              alpha = 0.25) +  
