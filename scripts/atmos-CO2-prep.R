...
## read in data
if (!file.exists("../data/privateEsther-CO2-Hourly.DAT")) {
...
names(comb.pdat)[which(names(comb.pdat)=='Fitted')] <- 'CO2'
...
tempplot <- ggplot(comb.pdatnorm, aes(x = DayofYear, y =HourEnding, z=temp)) + #, z=Fitted
  ...
  ylab("Hour") + xlab("Day of Year") +
  guides(guide=guide_legend(title.vjust = 0.1))
## FIXME -- model seems to need an interaction for year and day of year ... 
