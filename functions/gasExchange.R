...
gasExchange <- function(temp, cond, ph, dic, alt, kpa, wind, salt) { 
    ...
    r2d <- -6.44
    r2e <- -196.4
    if (file.exists("data/localc.rds")) { # constant for line 81 
      localc <- readRDS("data/localc.rds")
    } else {
      stop("get localc from dropbox") # does stop() get me out of gasExchange()?
    }
